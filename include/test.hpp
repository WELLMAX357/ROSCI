...
public:
    CZHandNode() : Node("cz_hand"), can_(this, "can0_to"), servo_(&can_, 0), air_(&can_, 0, false) {
        joy_sub_ = this->create_subscription<Joy>("/joy", 50, std::bind(&CZHandNode::joyCallback, this, std::placeholders::_1));
        oa_timer_ = this->create_wall_timer(std::chrono::milliseconds(50), std::bind(&CZHandNode::oaCallback, this));
        log_timer = this->create_wall_timer(std::chrono::milliseconds(1000), std::bind(&CZHandNode::logger, this));
    ...
    void oaCallback() {
        ...
        switch (phase_) {
            case Phase::NONE:
                // 0 脱力
                message.air_4 = 1;
//                servo_.sendRadICS(0, this->get_parameter("none0").as_double());
//                servo_.sendRadICS(1, this->get_parameter("none1").as_double());
//                servo_.sendRadICS(2, this->get_parameter("none2").as_double());
            ...
            case Phase::PREPARE:
                // 1 OB保持
                message.air_4 = 0;
//                servo_.sendRadICS(0, this->get_parameter("prepare0").as_double());
//                servo_.sendRadICS(1, this->get_parameter("prepare1").as_double());
//                servo_.sendRadICS(2, this->get_parameter("prepare2").as_double());
                servo_.sendFreeICS(0);
                servo_.sendFreeICS(1);
                servo_.sendFreeICS(2);
                break;
...
//                servo_.sendRadICS(2, 0 * DegToRad);
            ...
            case Phase::PREPARE_RELEASE:
                // 6 鉄板解除
                message.air_4 = 0;
                servo_.sendRadICS(0, this->get_parameter("prepare_release0").as_double());
                servo_.sendRadICS(1, this->get_parameter("prepare_release1").as_double());
                servo_.sendRadICS(2, this->get_parameter("prepare_release2").as_double());
//                servo_.sendRadICS(0, -23 * DegToRad);
